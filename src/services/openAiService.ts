...
/**
 * AI Service using OpenAI for intelligent SQL query generation
...
export interface QueryStep {
  ...
  reasoning?: string; // NEW: Track reasoning for step modifications
  isRemoved?: boolean; // NEW: Flag for removed steps
}
...
/**
 * AI Service class for intelligent SQL generation using OpenAI
 */
...
/**
 * IMPROVED: Clean and enrich database schema using AI with batch processing (OpenAI version)
 */
export async function cleanAndEnrichSchemaOpenAI(
  rawSchema: any[],
  databaseType: string = "postgresql"
): Promise<{
  success: boolean;
...
Return a JSON response with the following structure:
{
  "cleanedSchema": [
    {
      "tableName": "exact_table_name",
      "tableDescription": "Business description",
      "isRelevant": true,
      "category": "Category",
      "columns": [
        {
          "columnName": "exact_column_name",
          "dataType": "exact_data_type",
          "isPrimaryKey": false,
          "isForeignKey": false,
          "referencedTable": "table_name_if_fk",
          "isNullable": true,
...
/**
 * Main function to run AI-powered query using OpenAI
 * This is the primary interface for the OpenAI service
...
export async function runOpenAIQuery(
  ...
  return OpenAIService.runAIQuery(
    question,
    databaseId,
    databaseType,
    executeSQL,
